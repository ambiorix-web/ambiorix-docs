...
1. In that `.service` file place the following configuration, it creates a service that runs the application at the defined path (`/path/to/app`).
    ...
    **[Unit]** - Metadata and dependencies.
    ...
    - `After=network.target` - Start this service after the network subsystem is initialized.
      This means network interfaces exist and systemd's network management has started, but
      doesn't guarantee interfaces have IP addresses yet. Sufficient for most local web apps
      binding to `0.0.0.0` or `127.0.0.1`.
