...
Send files using the `send_file()` method:

```r
app$get("/file", function(req, res) {
  res$send_file(file = "path/to/file.html")
})
```

The `file` argument is the path to your file.

When using this method, you might want to pair
it with the [Content-Disposition](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Disposition) header:

```r
app$get("/iris", function(req, res) {
  res$header(
    "Content-Disposition",
    "attachment; filename=iris-tail.pdf"
  )
  res$send_file(file = "xyz.pdf")
})
```
...
```r
app$get("/cute-cat", function(req, res) {
...
```r
app$get("/cute-cat-png", function(req, res) {
  res$png(file = "/path/to/local/png/file")
})

app$get("/cute-cat-jpeg", function(req, res) {
...
```r
app$get("ggplot", function(req, res) {
  # make the plot:
  p <- ggplot2::ggplot(
    data = iris,
    mapping = ggplot2::aes(
...
Get set cookies by their name via the `cookie` item of the request object:

```r
cookie_get <- function(req, res) {
  value <- req$cookie$today %||% ""
  msg <- paste("The value of cookie 'today' is:", value)
...
- If not specified, `status` defaults to `302 "Found"`.
  ...
  302 is appropriate for most application redirects eg. post-login, form submissions,
  conditional routing etc. as it doesn't pollute browser caches or tell search engines
  the original URL is gone forever.
...
A pre-render hook runs before the `render()` method. Pre-render
