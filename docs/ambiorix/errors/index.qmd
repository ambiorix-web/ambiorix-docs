...
If you created your project using the [ambiorix.generator](/docs/generator/) or the [ambiorix-cli](/docs/cli/), ambiorix defaults to using the following handler on error:

```r
render_500 <- function(req, res, error) {
  res$status <- 500L
...
```r
error_handler <- function(req, res, error = NULL) {
  if (!is.null(error)) {
